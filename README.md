...
- **部署期**：用于把远程模板或项目仓库拉到本地、完成初始化、切分支、更新代码。这个阶段可以由人、AI、脚本或命令行完成，不要求由影刀执行。
- **部署工具**：`git_controller.py` 只是一个可选的部署辅助工具，用于 clone / 切分支 / pull；也可以直接由 AI 或人工使用 Git 完成同样工作。
- **执行期**：`runner.py` 只做业务执行，不自动切 Git，不承担项目初始化职责。
